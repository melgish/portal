...
<script lang="ts">
	...
	const onSubmit = async ({ userName, password }: SignInParam) => {
		error = undefined;
		const body = JSON.stringify({ userName, password });
		const headers = { 'content-type': 'application/json' };
		try {
			const rs = await fetch('api/auth/signIn', { method: 'POST', body, headers });
			if (rs.ok) {
	...
	const { errors, form, isValid, handleSubmit, handleChange } = createForm<SignInParam>({
